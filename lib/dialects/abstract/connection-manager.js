      ...
      this.pool = Pooling.Pool({
        ...
        create: (callback) => {
          this._connect(config).nodeify((err, connection) => {
              ...
              if (!err && config.dialectOptions && config.dialectOptions.onConnect && typeof(config.dialectOptions.onConnect) === 'function') {
	          config.dialectOptions.onConnect(connection, err, callback); // Call the callback function inside the hook function like in the 'else' in order to work properly
	      } else {
